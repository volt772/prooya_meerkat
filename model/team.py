...
class TeamModel:
    ...
    def put(self, data):
        ...
        query = """
            ...
            WHERE pid='%s' AND year='%s'
            RETURNING id
            """ % (TEAMS, team, score, data["pid"], data["year"])
    ...
    def get_details(self, data):
        ...
        query = """
            SELECT r.getscore, r.lostscore, r.regdate, r.result, r.versus
            FROM %s AS u
            JOIN %s AS r
            ON r.pid = u.id
            WHERE u.pid = '%s' %s
            ORDER BY r.regdate DESC
            """ % (USERS, RECORDS, email, versus_condition)
